...
public interface FriendsCustomRepository {
	...
	@Query(value = " WITH FRIENDS_DATA AS (" + 
			"	   SELECT " + 
			...
			"			else 3 END AS accept_status," + 
			"			addeddate," + 
			"			invitationacceptdate, requestuniueid, id " + 
			"			FROM friend_list " + 
			"			WHERE (invitedby = :userid OR friendsid = :userid)" + 
			"			)" + 
			"			SELECT  A.friendsid," + 
			"					B.emailaddress," + 
			"					B.mobilenumber," + 
			...
			"					B.fullname ," + 
			"					A.addeddate," + 
			"					A.invitationacceptdate," + 
			"					A.accept_status, A.id" + 
			"					" + 
			"			FROM  FRIENDS_DATA A JOIN user_master B  ON A.friendsid = B.id",
		    	   nativeQuery = true, name = "getMyFriendList")
